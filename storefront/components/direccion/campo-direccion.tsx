...
function CampoTexto({
  ...
  etiqueta = "Dirección",
  requerido = false,
  claseCampo = "rounded border-2 border-negro px-3 py-2",
  claseEtiqueta = "flex flex-col gap-1 text-sm font-bold",
  defaultValue = "",
  onInput,
  onKeyDown,
  onFocus,
...
function Mapa({
  ...
  return (
    <div
      ref={contenedor}
      aria-hidden
      className={`w-full overflow-hidden rounded-lg border-2 border-negro ${
        visible ? "h-44" : "hidden"
      }`}
    />
  );
...
/**
 * Campo de dirección anclado a Google Maps, para el checkout del sitio.
 *
 * **Copia de `frontend/components/direccion/campo-direccion.tsx`** (ADR-053,
 * ADR-072) con la paleta de la marca. Se copia y no se comparte porque las dos
 * apps de Next son proyectos separados, con su `package.json` y su imagen: un
 * paquete común obligaría a montar workspaces de npm para siete archivos. Mismo
 * criterio —y mismo precedente— que `lib/google-maps.ts`. Queda anotado en
 * `docs/roadmap/deuda/modulo-storefront.md`: si aparece un tercer consumidor,
 * ahí sí conviene el paquete.
 *
 * Una sola caja, no dos: el `<input>` de siempre busca sugerencias de Google
 * mientras se teclea y las muestra en un desplegable propio. Elegir una escribe
 * el texto, guarda el ancla y centra el pin del mapa de abajo, con arrastre para
 * corregir la puerta exacta cuando Google la deja a media cuadra.
 *
 * **Se puede escribir una dirección que Google no conoce.** En Tarapoto hay
 * calles así, y un pedido no puede depender de que un tercero conteste.
 *
 * **Editar el texto a mano suelta el pin.** Si el texto dijera una calle y las
 * coordenadas otra, el reparto iría al lugar equivocado y cobraría la distancia
 * equivocada. El backend aplica la misma regla por su cuenta
...
export function CampoDireccion({
  ...
  return (
    <div className="flex flex-col gap-1.5">
      ...
      <p className="text-xs text-humo" role="status">
