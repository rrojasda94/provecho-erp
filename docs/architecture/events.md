...
| Venta para reparto | `sales` | `delivery` (dirección, coordenadas, cliente y si ya está lista para asignarse a una ruta — ADR-098) | `sales/application/queries_publicas.py::venta_para_reparto` — sucursal, modalidad, canal, dirección/ubicación, distancia de reparto ya cotizada, si tiene plataforma externa, si todos los ítems llegaron a `listo`, si ya está entregada | interno (llamada en proceso; el endpoint que lo expone exige `delivery.leer`) |
| Ventas para reparto | `sales` | `delivery` (tablero: "sin asignar" — ADR-098/101) | `sales/application/queries_publicas.py::ventas_para_reparto` — ventas delivery, sin plataforma externa, no anuladas, con al menos un ítem no `entregado` (no hace falta que esté `listo` — ADR-101), de las sucursales pedidas | interno (llamada en proceso; el endpoint que lo expone exige `delivery.despachar`) |
| Trabajadores con cuenta | `rrhh` | `delivery` (candidatos a repartidor: solo un trabajador activo con usuario propio puede repartir — ADR-070) | `rrhh/application/queries_publicas.py::trabajadores_con_cuenta` — trabajador_id, usuario_id, nombre, sucursal_id, cargo, solo activos con cuenta | interno (llamada en proceso; el endpoint que lo expone exige `delivery.gestionar_repartidores`) |
| Cuenta de un trabajador | `rrhh` | `delivery` (resolver `usuario_id`/teléfono al dar de alta un repartidor) | `rrhh/application/queries_publicas.py::cuenta_de_trabajador` — usuario_id, nombre, telefono, empresa_id, sucursal_id, o `None` si el trabajador no tiene cuenta | interno |
| Usuarios con un permiso | `users` | `delivery` (a quién avisarle en la bandeja in-app cuando se registra una entrega o termina una ruta, por permiso y no por rol — ADR-101) | `users/application/queries_publicas.py::usuarios_con_permiso` — ids de usuarios con el código de permiso (o el comodín `*`) en una sucursal, vía `UsuarioSucursal`/`UsuarioRol`/`RolPermiso` | interno |
...
|--------|--------|--------------|-----------------|--------|--------|
| `sales.venta_confirmada` | sales | inventory, accounting, marketing (atribución lead→venta) | venta_id, sucursal_id, cliente_id (opcional), items[] (receta_id, cantidad, empaque_articulo_id, sin_articulo_ids, valores_variante_ids, **producto_comercial_id**, **importe**), total | Al confirmar la venta. `cliente_id` se agregó 2026-08-01 para la atribución de marketing; `sin_articulo_ids` (restas de la línea) el 2026-08-09 — ausente = la línea no quitó nada. `producto_comercial_id` e `importe` el 2026-08-30 (ADR-086): `accounting` reparte el ingreso entre las cuentas de la categoría de cada producto, y el importe **no se puede recalcular afuera** —sale de la lista de precios, la promoción, el recargo de la variante y el descuento de línea—. Aditivos, como todo lo anterior: un consumidor que los ignore se comporta como antes | RN-COM-001, RN-PRD-002, RN-MKT-003 |
| `sales.consumo_personal_registrado` | sales | inventory (descuenta como `consumo_interno` y valoriza), **reports** (emisión del catálogo: llega a Gerencia y Contabilidad) | venta_id, sucursal_id, cliente_id (siempre null), items[], total (siempre "0"), tipo, consumo_motivo, numero_orden, consumo_autorizado_por | Se confirma la comida del personal (RN-COM-025). **Evento propio y no `venta_confirmada`**: ese lo consumen `accounting` como ingreso y `marketing` como venta atribuible, y un consumo de personal no es ninguna de las dos | RN-COM-025/026/027, ADR-033 |
| `sales.venta_pagada` | sales (`application/ventas.py`, al cubrir la última cuenta con saldo) | **accounting** (`on_venta_pagada`, ADR-089: cancela la cuenta por cobrar 1212 contra caja/banco según el medio; una venta al crédito no asienta nada) | venta_id, sucursal_id, total, medios[] (tipo, monto — lo cobrado agrupado por tipo de medio de pago) | La venta queda en `pagada`: ninguna cuenta (grupo de cobro) conserva saldo. Cada cuenta emite su comprobante al cubrirse, pero el evento sale **una sola vez**, con la venta entera. Reemplaza a `sales.pago_registrado`, que esta tabla documentaba y **nunca se publicó**: el nombre real siempre fue `venta_pagada`, y hasta ADR-089 se publicaba con dos claves sin que nadie lo escuchara | RN-COM-002, ADR-089 |
| `sales.comprobante_emitido` | sales | **accounting** (asiento del débito fiscal desde 2026-08-29, ADR-081) | comprobante_id, venta_id, empresa_id, tipo, serie_numero, **total**, **gravado_igv** | Comprobante aceptado por SUNAT (Factiliza). `total` es el importe de **este** comprobante, no el de la venta entera: una cuenta dividida emite uno por grupo de cobro (RN-COM-018) y mandar el total en cada uno hacía que `accounting` reconociera el IGV una vez por comprobante sobre la venta completa. `gravado_igv` es la casilla del cobro (`null` = el régimen de la empresa) | RN-COM-003, RN-IMP-001 |
...
| `sales.venta_anulada` | sales | inventory, accounting, **reports** (acto de autoridad, RN-AUD-005), **delivery** (cancela la `entrega` si estaba `pendiente`/`asignada`; si ya estaba `en_ruta` no la cancela sola — notifica in-app a quien creó la ruta, RN-DLV-006) | venta_id, sucursal_id, usuario_id, items[] (receta_id, cantidad, sin_articulo_ids) | Al anular. Las restas viajan para que la reposición devuelva **solo lo que se consumió** (RN-PRD-019) | RN-GEN-002 |
...
| `sales.carrito_abandonado` | sales | — (analítica) | carrito_id, canal, paso, motivo (opcional) | **Planificado — sin publicador en código todavía.** Al abandonar sin confirmar | RN-COM-013 |
| `sales.pedido_listo` | sales (PROC-OPE-002) | — (pantalla de despacho, analítica de tiempos). `delivery` NO lo consume a propósito (ADR-098): el tablero de reparto resuelve "sin asignar" con la consulta activa `sales.queries_publicas.ventas_para_reparto`, no con este evento pasivo — así un pedido que pasa a delivery también aparece, esté o no `listo` (ADR-101) | venta_id | Todos los ítems del pedido alcanzan `listo` | RN-CUP-005 |
| `sales.venta_entregada` | sales (PROC-OPE-002) | marketing* (hoy lee la entrega por consulta, `venta_para_encuesta`), accounting* (cobro al finalizar en mesa) | venta_id, sucursal_id, modalidad, cliente_id (opcional), repartidor_externo_plataforma (opcional), entregado_por | El pedido queda en manos del cliente | RN-CUP-005/006/007/009, RN-COM-007 |
| `delivery.ruta_iniciada` | delivery | delivery (dispara aviso "en camino" por entrega) | ruta_id, sucursal_id, repartidor_id, entrega_ids[], venta_ids[] | El despachador inicia la salida: hay ≥1 parada, repartidor activo, todas las paradas `lista`, `hora_salida` | RN-DLV-002/005 |
| `delivery.entrega_registrada` | delivery | **sales** (marca la venta entregada vía `cumplimiento.registrar_entrega`, ADR-098), delivery (dispara aviso "entregado" al cliente y, desde ADR-101, notificación in-app a caja para cobrar) | entrega_id, venta_id, sucursal_id, ruta_id, repartidor_id, repartidor_usuario_id, fecha_entrega | El repartidor (o despacho) confirma la entrega en la puerta | RN-CUP-005/007, RN-DLV-001 |
| `delivery.entrega_fallida` | delivery | delivery (dispara aviso al cliente y notificación in-app a quien creó la ruta) | entrega_id, venta_id, sucursal_id, ruta_id, repartidor_id, motivo, intento | Cliente ausente, dirección errada o rechazo en la puerta — nunca marca la venta entregada | RN-CUP-008, RN-DLV-003 |
| `delivery.ruta_finalizada` | delivery | delivery, desde ADR-101 (notificación in-app a caja y cocina — antes sin consumidor) | ruta_id, sucursal_id, repartidor_id, distancia_m, duracion_seg, entregadas, fallidas | Todas las paradas de la ruta quedaron resueltas (entregadas, fallidas o canceladas) | RN-DLV-004 |
...
| `rrhh.trabajador_cesado` | rrhh | users* (desactivar el `usuario` ligado) | trabajador_id, empresa_id, usuario_id (nulo si no tiene cuenta), fecha_cese | El trabajador pasa a `cesado` con su fecha de cese | — |
| `rrhh.contrato_laboral_firmado` | rrhh | — (sin consumidor todavía; ver deuda `modulo-rrhh`) | contrato_laboral_id, trabajador_id, fecha_firma | El contrato laboral pasa de borrador a `firmado` | — |
| `rrhh.boleta_pago_emitida` | rrhh | accounting* (asiento de planilla) | boleta_pago_id, trabajador_id, periodo, neto_pagar | Se emite la boleta de pago del periodo (idempotente por `idempotency_key`) | — |
| `rrhh.liquidacion_bss_pagada` | rrhh | accounting* (asiento de la liquidación) | liquidacion_bss_id, trabajador_id, total, dentro_de_plazo | Se paga la liquidación de beneficios sociales del cese (idempotente por `idempotency_key`). `dentro_de_plazo` dice si se pagó en el plazo legal | — |
| `rrhh.solicitud_permiso_aprobada` | rrhh | — (sin consumidor todavía; ver deuda `modulo-rrhh`) | solicitud_permiso_id, trabajador_id, tipo | Se aprueba una solicitud de permiso — el rechazo no publica nada | — |
| `rrhh.amonestacion_emitida` | rrhh | — (sin consumidor todavía; ver deuda `modulo-rrhh`) | amonestacion_id, trabajador_id, tipo | Se emite una amonestación (memorándum) al trabajador | — |
| `inventory.stock_consumido` | inventory | — (auditoría) | venta_id | Tras descontar el insumo de una venta (`sales.venta_confirmada`) o de un consumo de personal, si al menos una línea movió stock. Corregido el 2026-09-18: esta fila documentaba `almacen_id, articulo_id, cantidad, ref`, que el publicador nunca mandó — el detalle está en `movimiento_inventario` con `referencia = venta_id`. El consumo de producción no lo publica (tiene su propio `production.consumo_registrado`) | RN-INV-003 |
| `inventory.stock_repuesto` | inventory | — (auditoría; sin consumidor) | venta_id | Espejo de `stock_consumido`: se repuso el insumo de una venta anulada, de líneas anuladas o de una nota de crédito que pidió reponer, devolviéndolo a los lotes de los que salió. Lo publica el mismo `_mover` de `application/listeners.py` cuando el signo es positivo | RN-PRD-019, RN-GEN-002 |
| `inventory.stock_bajo_minimo` | inventory | **reports** (desde 2026-08-08, antes `users`), **production** (`application/listeners.py::on_stock_bajo_minimo`, 2026-09-09 — crea la orden por ajuste de necesidad si es una subreceta con BOM y la empresa tiene una sola cocina de producción) | almacen_id, sku_id, cantidad, stock_minimo, **usuario_id** (2026-08-09, ADR-036: quién hizo el movimiento que cruzó el mínimo; nulo si no vino de una persona) | **Al cruzar** el mínimo, no cada vez que está por debajo: con el stock ya bajo, un evento por venta vuelve ruido la alerta. Reponer y volver a caer avisa de nuevo. `sku_id` y no `articulo_id` porque la fila de stock —y el mínimo— son por SKU | RN-PRD-007, RN-INV-008 |
...
| `purchases.compra_recibida` | purchases | inventory, accounting | **orden_compra_id**, **almacen_destino_id**, items[] (articulo_id, cantidad, costo_unitario, lote_codigo, fecha_vencimiento) | Al recibir mercadería; los dos últimos campos solo los usa inventory si el artículo controla lote (RN-VNC-002). Los dos primeros se corrigieron el 2026-08-31: esta fila decía `oc_id, almacen_id`, nombres que el publicador **nunca usó** — quien escribiera un consumidor leyendo la doc recibía `KeyError`. El artículo recibido que no tenga SKU no mueve stock: el listener lo saltea con una incidencia `sin_sku` (RN-PRD-006) | RN-CMP-003 |
| `purchases.comprobante_conforme` | purchases | accounting | comprobante_id, orden_compra_id, proveedor_id, empresa_id, condicion_pago, sujeto_spot, porcentaje_deteccion, monto, **gravado_igv**, `total_documento` | Compras da conformidad al comprobante; accounting encola el pago (`movimiento_dinero` pendiente) **y asienta el crédito fiscal** (ADR-081): la recepción asentó la compra sin IGV porque el comprobante todavía no existía, y el crédito solo se toma con el comprobante válido y anotado. `gravado_igv` lo marca quien tiene la factura del proveedor delante. `monto` es la **base** de lo recibido, que es lo que la plantilla del PCGE espera (`monto_es="base"`): apuntarlo al total facturado duplicaría el IGV que este mismo asiento agrega. `total_documento` es el importe que declara el papel, aditivo y **todavía sin consumidor** — existe para la conciliación (ADR-085) | RN-CMP-005, RN-CMP-014, RN-IMP-001 |
| `purchases.requerimiento_activo_recibido` | purchases | assets (`on_requerimiento_activo_recibido`, da de alta el activo) | orden_compra_id, requerimiento_activo_id, empresa_id, sucursal_id, id_interno, nombre, categoria, marca, modelo, costo_estimado, vida_util_meses, proveedor_id, solicitado_por, fecha_compra | Al recibir (total, nunca parcial) una OC tipo `activo` (ADR-099). Un fallo de `assets` (`id_interno` duplicado) queda en el log y no revierte la recepción, mismo criterio que `purchases.compra_recibida` con `inventory` | ADR-099 |
| `purchases.oc_anulada` | purchases | — (sin consumidor todavía) | orden_compra_id, usuario_id | Se anula una OC que todavía no tiene recepción registrada (con recepción no admite anulación directa). `usuario_id` es quien la anuló | RN-CMP-001 |
| `purchases.caja_chica_rendida` | purchases | accounting | rendicion_id, gasto_total, efectivo_restante, diferencia | **Planificado — sin publicador en código todavía.** Al cerrar la rendición semanal de caja chica | RN-CMP-017 |
| `purchases.evaluacion_proveedor_actualizada` | purchases | — (informativo) | proveedor_id, indicador_automatico | **Planificado — sin publicador en código todavía.** En cada recepción (cumplimiento, conformidad, variación de precio) | — |
| `production.consumo_registrado` | production | inventory (descuenta insumos vía FEFO, tipo `consumo_produccion`) | orden_produccion_id, almacen_id, items[] (articulo_id, cantidad, **costo_unitario**) | Al registrar el consumo real de insumos de una orden, transición `borrador→en_proceso`. Fila agregada el 2026-09-09: el evento se publica desde 2026-07-25 y nunca tuvo entrada acá — la regla de este mismo archivo (definir la fila antes de publicar) no se había cumplido. `costo_unitario` se agregó el mismo día (bloque `feat/produccion-costeo-real`): sin `costo_unitario` explícito en la línea, ya viaja el `costo_promedio` vigente del artículo, no el que tipeó quien registró el consumo | RN-PRD-018 |
| `production.orden_completada` | production | inventory (suma stock del artículo terminado, tipo `produccion_entrada`, recalcula `costo_promedio`, crea `lote` si el artículo lo controla) | orden_produccion_id, almacen_id, articulo_id, cantidad_producida, costo_unitario, **fecha_vencimiento** (opcional, ISO), **lote_codigo** (opcional) | Al completar con resultado `conforme`. Corregido el 2026-09-09: esta fila decía `orden_id, articulo_id, cantidad`, tres de cinco nombres que el publicador nunca usó — mismo tipo de desalineación ya corregida en `purchases.compra_recibida` el 2026-08-31. `fecha_vencimiento`/`lote_codigo` se agregaron el mismo día (bloque `feat/produccion-lote-trazabilidad-auditoria`): sin ellos el lote nace igual, solo que sin vencimiento y FEFO lo trata como FIFO | RN-PRD-003, RN-VNC-001 |
| `production.no_conformidad_detectada` | production | reports (emite el reporte y lo distribuye) | orden_produccion_id, **almacen_id** (agregado 2026-08-08: de ahí sale la empresa y la sucursal del hecho), resultado (`no_conforme_reprocesado`\|`no_conforme_desechado`), **registrado_por** (2026-08-09, ADR-036: quién cerró la orden con el control de calidad en la mano), **evidencia_id** (agregado 2026-09-09, bloque `feat/produccion-evidencia-como-archivo`: `orden.evidencia_archivo_id` si ya se adjuntó — `null` si no). El escalamiento ya está modelado —`reporte_escalamiento`, ADR-036— y se abre **desde el reporte**, no desde este evento; con `evidencia_id` en el payload, `reports.application.escalamientos.abrir` lo usa como default de `reporte_escalamiento.evidencia_id` sin que el usuario lo vuelva a pegar a mano (RN-PRD-015 una sola vez) | Al registrar control de calidad no conforme. `no_conforme_reprocesado` no genera merma ni asiento, solo el detalle de la corrección en el reporte de escalamiento. `no_conforme_desechado` dispara además `production.orden_desechada` (fila siguiente) — el asiento contable de este hallazgo | RN-PRD-013/014/015 |
| `production.orden_desechada` | production | accounting (asiento por plantilla PCGE, ADR-100) | orden_produccion_id, almacen_id, articulo_id, merma_cantidad, merma_motivo, **monto**, registrado_por | Solo cuando `no_conformidad_detectada` cierra en `no_conforme_desechado` (nunca en reproceso). `monto = costo_insumos` de la orden — la mano de obra queda afuera porque ya se reconoce como gasto de planilla, aparte. **No** es `inventory.merma_registrada`: el producto terminado de una orden desechada nunca llegó a existir como stock, así que no hay reserva que apartar ni desechar (ver ADR-100) | RN-PRD-014/015 |
| `production.equipo_frio_fuera_rango` | production | reports (alerta a Gerencia y Cocina) | checklist_id, almacen_id, equipo, temperatura_c, rango_min, rango_max | Al registrar el checklist de turno (`POST /production/checklists`), un equipo de frío fuera de rango — el checklist entero queda `bloqueado`. Corregido el 2026-09-09 (bloque `feat/produccion-checklist-inocuidad`): esta fila documentaba `cocina_produccion_id, equipo_id, rango_esperado`, nombres que ningún publicador llegó a usar — el evento nunca se publicó hasta este bloque | RN-CDP-005 |
| `production.cocina_bloqueada` | production | reports (alerta a Gerencia y Cocina) | checklist_id, almacen_id, turno, fecha, motivo | Al registrar el checklist de turno: algún equipo de frío fuera de rango o indicio de plaga (RN-CDP-002/005). Mientras la cocina siga así, `crear_orden_produccion` y `registrar_consumo` rechazan con 409 (`CocinaBloqueada`) | RN-CDP-002, RN-CDP-005 |
| `production.reporte_produccion_generado` | production | reports (informa a Gerencia y Cocina, sin actor — lo genera un barrido) | reporte_produccion_id, almacen_id, jornada, merma_total, desperdicio_total | Al cerrar la jornada (barrido `production.generar_reportes_de_jornada_vencidos`, cada 15 min según `hora_cierre_jornada` por empresa) o al generar a demanda (`POST /reportes-jornada/generar`). Pendiente de visado por el jefe de cocina | RN-DOC-010 |
| `supervision.informe_diario_generado` | supervision | reports (informa a Gerencia, sin actor — lo genera un barrido) | informe_id, sucursal_id, fecha, total, completadas, vencidas, fotos_invalidas | Al cerrar la jornada de tareas de apertura/cierre de una sucursal (barrido `supervision.cerrar_jornadas_vencidas`, cada 15 min según `supervision_hora_cierre_jornada`) o al generar a demanda (`POST /supervision/informes/generar`). Toda tarea que siga `pendiente` al cerrar pasa a `vencida` | RN-SUP-007 |
...
| `users.usuario_creado` | users | — | usuario_id, tipo | **Planificado — sin publicador en código todavía.** Al crear usuario | — |
| `organizacion.empresa_creada` | users (`application/organizacion.py`; prefijo `organizacion` y no `users` porque el hecho es de la estructura del grupo, no de una cuenta) | **accounting** (`on_empresa_creada`, ADR-089: le siembra el plan de cuentas PCGE; `importar_pcge` es idempotente) | empresa_id, razon_social | Al dar de alta una empresa. Antes el PCGE se importaba con un botón que nadie sabía que había que apretar, y sin cuentas cada asiento automático se descartaba en silencio | ADR-089 |
...
| `reports.reporte_emitido` | reports | users (llena la bandeja: una fila de `notificacion` por destinatario; si `canal="email"`, además manda un correo a quien tenga uno registrado) | reporte_emitido_id, codigo, titulo, cuerpo, nivel, **canal** (agregado 2026-09-09, `regla_distribucion.canal` de la regla que resolvió la entrega), sucursal_id, referencia_tipo, referencia_id, destinatarios[] | Cuando una emisión del catálogo resolvió al menos un destinatario. **No se publica si no hay destinatarios**: no habría nada que repartir (el hueco queda registrado del lado de `reports`) | RN-REP-001..008, ADR-033 |
...
| `assets.mantenimiento_proximo` | assets | **reports** (emite y distribuye a gerencia/compras) | plan_id, activo_id, empresa_id, sucursal_id, nombre, plan_nombre, proxima_fecha, proximo_km | El barrido diario `assets.barrer_vencimientos` encuentra un plan que entra en su ventana de aviso (por fecha o por km, RN-MNT-005). Se publica **una sola vez** por ventana — `aviso_proximo_en` guarda cuándo | RN-MNT-001/005 |
| `assets.mantenimiento_vencido` | assets | **reports** | plan_id, activo_id, empresa_id, sucursal_id, nombre, plan_nombre, proxima_fecha, proximo_km | El plan ya pasó su fecha o km de mantenimiento | RN-MNT-001/005 |
| `assets.documento_por_vencer` | assets | **reports** (gerencia/contabilidad/rrhh) | documento_id, empresa_id, sujeto_tipo, sujeto_id, tipo_documento, fecha_vencimiento | El barrido diario encuentra un `documento_vigencia` (SOAT, licencia de funcionamiento, carné de sanidad, etc.) que entra en su ventana de aviso. Una sola vez por ventana | RN-VIG-001..004 |
| `assets.documento_vencido` | assets | **reports** | documento_id, empresa_id, sujeto_tipo, sujeto_id, tipo_documento, fecha_vencimiento | El documento ya pasó su fecha de vencimiento | RN-VIG-001..004 |
...
`*` = consumidor previsto, sin listener en código todavía. Reglas referenciadas en
...
> **Nota (2026-09-18)**: barrido de `event_bus.publish` en todo `src/` contra
> esta tabla. Se regularizan diez eventos que se publicaban sin fila
> (`sales.venta_pagada`, `organizacion.empresa_creada`, `purchases.oc_anulada`,
> `inventory.stock_repuesto` y seis `rrhh.*`); `sales.pago_registrado` se
> reemplaza por `sales.venta_pagada`, que es lo que `accounting` escucha, y
> cuatro filas sin publicador quedan marcadas como planificadas. Las dos
