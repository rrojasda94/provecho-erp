...
| `inventory.devolucion_a_proveedor` | inventory | purchases* | devolucion_id, almacen_id, referencia_id (proveedor), motivo, destino, reporte_dirigido_a, items[] (sku_id, lote_id, cantidad) | Al registrar la devolución (ADR-028). La mercadería ya salió del almacén con el lote declarado. **Sin consumidor todavía**: el reclamo y la nota de crédito al proveedor son deuda de `purchases` | RN-INV-019/020 |
| `inventory.devolucion_de_cliente` | inventory | — (comercial, por reporte) | devolucion_id, almacen_id, referencia_id (cliente), motivo, destino, reporte_dirigido_a, items[] | Un cliente devolvió: la mercadería entró y `destino` ya decidió si volvió al estante (`reintegro`) o quedó apartada como merma (`desecho`/`auditoria`) | RN-INV-019/020 |
