...
| Nombre y cargo del trabajador | `rrhh` | `core/reportes` (rotular el ranking de venta por quien atendió) | `rrhh/application/queries_publicas.py::nombres_por_usuario` — `usuario_id` → nombre y cargo. **Solo eso**: remuneración, contratos y sanciones no salen de `rrhh` por ningún contrato. La cuenta se resuelve por persona (ADR-069): una persona recontratada puede matchear dos trabajadores, y gana el no cesado (o el de ingreso más reciente en empate) | `rrhh.leer` (vía el catálogo de reportes) |
