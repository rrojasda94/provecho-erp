...
### 6. Lo firma un encargado, con PIN
...
- `venta.estado` gana un quinto valor. Todo lector de estados —cierre de
  ...
  venta al agregar un filtro nuevo.
...
- El reporte formal de consumo de personal por sucursal/mes queda como deuda
  (catálogo cerrado, ADR-024). Hoy se lee por `GET /sales/ventas?tipo=
  consumo_personal` y por los movimientos `consumo_interno`.
