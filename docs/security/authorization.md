...
| — | **No aprueba órdenes de compra** ni **configura pantallas KDS**. Lo primero es una decisión de plata (decisión 2026-08-05); lo segundo es alta de infraestructura del local (decisión 2026-08-24, ADR-064): montar, renombrar o borrar una estación cambia por dónde pasa la comanda de todos los turnos, no solo del suyo. Opera lo que ya está montado |
...
| terminal_asistencia | `rrhh.asistencia_terminal` y nada más — cuenta de servicio del pad del local (ADR-064) |
...
| — | `rrhh.turno_gestionar` configura los turnos de trabajo del local y su hora límite de marcaje de salida (ADR-063). Es de RRHH y no de `organizacion.gestionar`: el horario laboral es materia de planilla, no infraestructura del local |
