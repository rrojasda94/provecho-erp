...
| — | `rrhh.terminal_gestionar` (ADR-073) autoriza o revoca el dispositivo que puede marcar asistencia por una sucursal — genera el código de activación, no lo tiene `terminal_asistencia` ni `supervisor`. Mismo criterio que `kds.configurar` frente a `kds.operar`: montar el dispositivo es alta de infraestructura, distinto de operar lo que ya está montado |
