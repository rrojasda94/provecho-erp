...
**Estado actual:** 13 módulos en código, desplegados en staging;
versión y avance por módulo en [`/ROADMAP.md`](../ROADMAP.md).
...
| | [openapi.json](architecture/openapi.json) | Contrato OpenAPI generado de la API (ADR-010) |
...
| | [adr/](architecture/adr/) | Decisiones de arquitectura — 001 modular monolith, 002 stack, 003 Izipay, 004 tenant, 005 Factiliza, 006 observabilidad, 007 backups y salud, 008 entrega continua, 009 modo offline del PDV, 010 contrato OpenAPI, 011 derechos ARCO, 012 dashboard + caja, 013 arquitectura frontend, 014 parámetros configurables por empresa, 015 lote y FEFO, 016 eventos post-commit, 017 jerarquía de errores compartida, 018 cobro dividido/mesa/descuento de orden, 019 conteo cíclico por categoría, 020 reserva/solicitud/transferencia, 021 atribución lead→venta y dueño de la encuesta, 022 restricciones de permiso, 023 variantes de producto y recetas en la ficha, 024 catálogo cerrado de reportes, 025 ciclo de caja completo, 026 paginación de colecciones, 027 guía de remisión, 028 merma como reserva y devolución, 029 encuesta por nodos y canal WhatsApp, 030 evaluación de agencia y acumulado de campaña, 031 audit_log transversal, 032 token de API para agentes, 033 módulo reports (emisión y distribución), 034 consumo de personal, 035 restas y lienzo de nodos de receta, 036 escalamiento de reportes y destinos accionables, 037 sistema visual, modo oscuro y accesibilidad, 038 grupos de opciones por variante en la carta, 039 rastro jerárquico y "volver" histórico, 040 abastecedor de respaldo, 041 reseteo de PIN y consulta de documento, 042 la variante hereda del padre, 043 orden abierta y ventana de corrección, 044 cadena de estaciones del KDS, 045 pinpad y bloqueo de pantalla del PDV, 046 carga masiva de recetas en dos fases, 047 dos suites de Playwright (`e2e` con techo, `uso` sin él), 048 el proxy del navegador pasa bytes, no texto, 049 la caja la abre el cajero (enmienda ADR-025), 050 el login también se teclea en el pinpad (enmienda ADR-045), 051 el requerimiento de la jornada, 052 exportar es la plantilla llena y la columna `ID` es la identidad, 053 la dirección se elige en el mapa, 054 el delivery se cobra por kilómetro, 055 atributos y variantes generadas (modelo Odoo), 056 líneas de receta condicionadas por variante y con UdM propia, 057 la matriz: el recetario como una hoja de cálculo, 058 el lienzo dibuja atributos y carga el árbol de una vez, 059 la caja se da de alta en Organización, 060 desplegar desde GitHub a pedido, 061 el cupón de la landing pública vive en `sales`, 062 el centro de labores no es el alcance de datos, 063 los atributos vuelven a la tabla (el lienzo se borró, superó a 058), 064 el turno de trabajo y la hora límite de marcaje, 065 el pad de asistencia y quién configura las pantallas, 066 los atributos llegan al PDV (supera §3 de 063), 067 el rollo de 80 mm es la medida única y el ticket lo arma el ERP, 068 la tarifa del delivery la fija Gerencia y el reparto se cobra, 069 las mesas se numeran solas y tienen plano, 070 la cuenta se liga al trabajador por la persona (supera el mecanismo de 065 §"Corregido el 2026-08-25"), 071 mover líneas no repone inventario ni reclasifica el asiento, 072 el campo de dirección es uno solo y el cliente por fin ancla (supera §"Dos cajas, no una" de 053), 073 la sesión se renueva sola y el PDV lee su sucursal fresca, 074 el borrador del PDV vive en el servidor, 075 el aumento es una tanda propia en el KDS (enmienda 043 y 044), 076 la promoción se aplica sola y no toca el descuento manual, 077 el sobrepago en efectivo se acepta y el vuelto se guarda (enmienda RN-COM-016), 078 el pedido sale de la cola al entregarse, no al facturarse (enmienda 044), 079 el marcaje se ancla al dispositivo, no a la red, 080 la landing pública tiene dominio propio y el proxy es quien lo acota, 081 el plan contable es el PCGE y los estados financieros salen del mayor, 082 la compra directa reutiliza `orden_compra`, no es un modelo nuevo, 083 BI autoservicio con Superset sobre vistas de solo lectura (enmienda ADR-024), 084 la sesión muere con el navegador y a las ocho horas quietas (enmienda 073), 085 la unicidad del comprobante depende de quién lo emitió, 086 la cuenta contable se configura en la categoría y se hereda, 087 la postulación se llena en el ERP y Google Forms queda como camino alterno, 088 la sesión muerta se anuncia una vez y corta el tráfico del navegador, 089 la empresa nace contable y el periodo se abre solo, 090 la guía de remisión se emite desde su documento origen, 091 el catálogo se corrige por pantalla, 092 la condición de una línea se valida al escribir, 093 el conteo cierra y arma el borrador, 094 la reposición vuelve al lote del que salió, 095 la consulta de documento se cachea con TTL, 096 el PATCH de organización distingue ausente de `null` explícito, 097 la suite corre contra Postgres y todo Enum vive con su CHECK, 098 el reparto propio es un módulo y avanza la venta por eventos, 099 módulo `assets`: activos, mantenimiento, combustible y documentos con vencimiento, 100 el desecho de producción asienta el costo de insumos, 101 el KDS despacha y el repartidor entrega, 102 módulo `supervision`: tareas de apertura/cierre, checklist y foto, 103 el sitio de marca es un módulo `storefront` y una app Next aparte, 104 la cuenta del sitio es una credencial aparte, vinculada por evento, 105 el pedido web es canal propio y el efectivo sigue siendo adelantado, 106 el PDV es un módulo propio del home, 107 grupos con pestañas en el sidebar, 108 kardex gráfico y próxima compra sugerida, 109 KDS, PDV y reparto como apps instalables |
| **engineering/** | [engineering-guide.md](engineering/engineering-guide.md) | Guía principal para constructores (humano + IA) |
| | [module-guide.md](engineering/module-guide.md) | Cómo crear un módulo: estructura, los 7 registros obligatorios (+5 condicionales) para activarlo, módulo de referencia |
...
| | [staging.md](engineering/staging.md) | Runbook y bitácora del servidor de staging: lo específico de ese droplet (crons, monitor, despliegues) |
| | [bi-superset.md](engineering/bi-superset.md) | Runbook del droplet del BI autoservicio (ADR-083): aprovisionar Postgres/Superset, bugs encontrados al ensayarlo |
...
| | [impresion-termica.md](engineering/impresion-termica.md) | Rollo de 80 mm: qué imprime el ERP, cómo se configura el membrete por marca, y cómo se quita el diálogo del navegador (`--kiosk-printing`) |
...
- [`/ROADMAP.md`](../ROADMAP.md) — tablero de estado por módulo, decisiones
  ...
  y el [plan de deuda de `production` del 2026-09-09](roadmap/deuda-production-2026-09-09.md).
