...
- ✅ 2026-08-29 **`contar_bajo_minimo` traía toda la tabla `stock` de la
  ...
  dashboard. Detectado al preparar el BI (ADR-083): la carga adicional de
...
- ⬜ **RLS del BI (Superset) sincronizada a mano con `Tenant`** (ADR-083): el
  ...
  juntos.
- ⬜ **Aprovisionamiento de Superset fuera de Alembic** (ADR-083 Fase C): la
...
- ⬜ **El BI (Superset) consulta la base viva, no una réplica de lectura**
  (ADR-083): mitigado con `bi_lector` de solo lectura y `statement_timeout`
  ...
  antes.
- ⬜ **Widget de embebido del BI sin construir** (ADR-083 Fase D):
...
- ✅ 2026-08-29 **La exportación baja lo que se ve, no el dataset
  completo** (ADR-083 Fase E). `POST /reportes/{codigo}/exportar` corre el
