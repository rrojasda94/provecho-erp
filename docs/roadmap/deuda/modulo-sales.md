...
- ⬜ **La promoción de cupón no es la `promocion` de §6** (2026-08-24,
  ADR-061). `promocion_cupon` hace una sola cosa —un porcentaje, una vigencia
  ...
  usa.
- ⬜ **Un cupón no se puede anular ni reemitir** (2026-08-24, ADR-061). Si un
...
- ⬜ **El código del cupón es adivinable** (2026-08-24, ADR-061). Es el DNI,
...
- 🔶 **Cumplimiento de pedido** (`PROC-OPE-002`, definido 2026-07-27):
  ...
  → `sales.venta_entregada`) para las tres modalidades. La **rama delivery
  con trazabilidad** (repartidor propio, hora de salida, resultado
  `entregado`/`fallido` con motivo — RN-CUP-007/008, evidencia) se separó
  como módulo aparte (2026-09-09, ADR-098): ver
  [`modulo-delivery.md`](modulo-delivery.md). Hasta que ese módulo tenga
  código, una entrega fallida sigue sin poder registrarse aquí: solo se
  marca el pedido entregado o no se marca nada.
...
- ✅ 2026-09-04 **Los otros botones que prometían 403** (auditoría §3).
  Cerrados en las tres ramas de la Ola 2 que les tocaban: «Ejecutar» /
  «Rechazar» pago (`fix/contabilidad-pagos-rbac`), «+ Asiento manual» /
  «Anular» asiento (`fix/contabilidad-asientos-rbac`), y «+ Nuevo
  trabajador» / «Editar» / «Cesar», «+ Nuevo artículo» / «Editar» /
  «Importar» y «+ Nueva devolución» / «Anular» (`fix/rbac-botones-resto`).
  Mismo arreglo que la jornada: `permisos` como prop y `tienePermiso` por
  acción. «Emitir» OC sobre umbral ya estaba gateado desde ADR-085 — la
  auditoría lo contó de más, eran cinco pantallas y no seis botones.
