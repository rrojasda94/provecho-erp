"""Config de Apache Superset para el BI autoservicio (ADR-081 Fase C).
...
# --- SSO contra Provecho (OAuth2, ADR-081 Fase B) -----------------------
...
# ADR-081 Fase B — la sesión de Provecho es una cookie que la API nunca ve).
...
# --- Feature flags -------------------------------------------------------
FEATURE_FLAGS = {
    # Guest tokens para embeber tableros en /dashboard (ADR-081 Fase D).
    ...
    # 1 GB elegido para este volumen de uso (ver ADR-081 "Dónde corre").
}

# Sin Celery (ver ADR-081): sin esto Superset intenta usar el broker por
...
# SQL Lab es la puerta que ningún permiso de fila cierra (ADR-081,
