...
| Migraciones Alembic | 🔶 al día 2026-08-08 | La BD dev —ahora el Postgres local del `docker-compose`— quedó verificada el 2026-08-08 en la cabeza de esta rama (`e7c390a5b41f`, devolución/merma/recepción parcial) con **cero deriva** según `python -m src.core.esquema`, y los seeders `seed` y `parametros` corrieron contra ella. Ojo: la base es **una sola para todos los worktrees**, así que otra rama en curso puede dejarla en una revisión que la tuya todavía no conoce — `esquema` entonces pide un `upgrade` que no corresponde. Es el mismo comportamiento que había con Supabase, no lo trajo la mudanza. Las primeras seis fueron transversal+org, slice Venta, cobro/caja, cliente opcional, slice auth/RBAC e inventory core; el detalle de cada slice posterior va en su fila. Tras aplicar una migración que suma permisos hay que correr `python -m src.seeders.seed` (idempotente): la migración crea tablas, no filas de RBAC. |
...
- ⬜ **El hub de sucursal sigue autenticándose con username + PIN**
  ...
  ADR-029). Migrarlo no es cambiar código: hay que emitir un token por
