...
| `delivery` (reparto propio: repartidores, rutas, seguimiento público) | 🔶 En curso | Slices 2-4 implementados 2026-09-09 (ADR-098): repartidores, tablero de despacho, ciclo completo de ruta/entrega con ruteo real (Google Routes) u heurístico, GPS de flota, enlace público de seguimiento con mapa y PWA instalable del repartidor (`/reparto`), convergiendo con `sales` por evento en los dos sentidos. Falta el tablero en el frontend del ERP y el aviso automático por WhatsApp. | [`modulo-delivery.md`](docs/roadmap/historial/modulo-delivery.md) |
