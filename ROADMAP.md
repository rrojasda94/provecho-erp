...
| Migraciones Alembic | 🔶 cabeza `b3f7d21a9c04` (2026-08-08, `token_agente`) | **Pendiente de aplicar en la BD dev** —desde 2026-08-08 el Postgres local del `docker-compose`—: correr `alembic upgrade head` + `python -m src.seeders.seed` (la migración crea la tabla; el permiso `organizacion.gestionar` lo siembra el seeder). Antes de eso, `python -m src.core.esquema` reporta deriva a propósito. Ojo: la base es **una sola para todos los worktrees**, así que otra rama en curso puede dejarla en una revisión que la tuya todavía no conoce — es el mismo comportamiento que había con Supabase, no lo trajo la mudanza. Las primeras seis fueron transversal+org, slice Venta, cobro/caja, cliente opcional, slice auth/RBAC e inventory core; el detalle de cada slice posterior va en su fila. Tras aplicar una migración que suma permisos hay que correr `python -m src.seeders.seed` (idempotente): la migración crea tablas, no filas de RBAC. |
