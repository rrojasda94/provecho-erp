...
| `storefront` (sitio de marca Charlie's Pizzas) | 🔶 En curso | PR1 (2026-09-17, ADR-101): app `storefront/` separada del ERP sirviendo `charlies.majambo.com.pe` — home, carta con búsqueda/filtros e ingredientes clicables, mapa de locales, "Nosotros" y "Trabaja con nosotros", todo en solo lectura contra `/api/v1/storefront/publico/*`; CMS de contenido y fotos de catálogo desde el ERP (`/web`). PR2 (2026-09-17, ADR-102): cuentas de cliente propias (email/clave + Google), direcciones, favoritos, "tu último pedido" en cuenta y home; credencial separada del ERP (`aud="storefront"`), vínculo a `cliente` por evento. PR3 (2026-09-17, ADR-103/104): carrito, checkout invitado o logueado, asignación automática de local por distancia/carga, ETA, boleta/factura, efectivo (cobra al entregar/recoger) e Izipay (adaptador falso), canal `web` nuevo en `venta`. Playwright, SEO y hardening de seguridad quedan para PR4. | [`modulo-storefront.md`](docs/roadmap/historial/modulo-storefront.md) |
