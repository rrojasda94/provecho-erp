...
| `delivery` (reparto propio: repartidores, rutas, seguimiento público) | ⏳ Pendiente | Especificado 2026-09-09 (ADR-098): separa del `sales` la entidad `entrega` con trazabilidad de repartidor, ruteo contra Google Routes con fallback heurístico, GPS de flota, enlace público de seguimiento y aviso automático por WhatsApp. Sin código todavía. | [`modulo-delivery.md`](docs/roadmap/historial/modulo-delivery.md) |
