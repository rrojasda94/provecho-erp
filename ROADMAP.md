...
- 🔶 **Ordenar y desarmar grupos de extras**: desarmar ✅ 2026-08-08
  (ADR-029) — `DELETE /productos/{id}/extras/{extra_id}` y
  `DELETE /productos/{id}/grupos/{grupo_id}`; borrar un grupo suelta sus
  extras en vez de borrarlos, porque el extra es un producto con su receta
  y su precio. Queda ⬜ **reordenar por arrastre**: `orden` se sigue
  tecleando en la ficha y en el lienzo de nodos. `@dnd-kit` ya está
  instalado (se usa en el PDV), así que es trabajo de pantalla, no de
  contrato.
- ⬜ **`receta_item.quitable`** (2026-08-08, ADR-029 §2): hoy **todo** insumo
  de la receta se puede pedir "sin". Se evaluó un flag para que Producción
  vetara restas absurdas ("pizza sin masa") y se descartó por ser una
  segunda fuente de la misma verdad, y porque el caso es inocuo — la resta
  no cambia el precio y cocina ve el pedido antes de prepararlo. Revisar si
  aparece un caso real donde quitar un insumo arruine el plato de forma
  cara; es una columna y un checkbox.
